...
function FlatList<T extends BaseItem>({
  ...
  if (isLoading) {
    return (
      loadingComponent || <div className="flatlist-loading">Loading...</div>
    );
  }
  ...
  if (isError) {
    return (
      errorComponent || <div className="flatlist-error">Error loading data</div>
    );
  }
  ...
  if (processedData.length === 0) {
    return (
      ListEmptyComponent ||
      emptyComponent || (
        <div className="flatlist-empty">No items to display</div>
      )
    );
  }
